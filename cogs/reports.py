...
class ReportsCog(commands.Cog):
    ...
    async def report_trends(self, interaction: discord.Interaction):
        ...
        # Phase 4.6: surface any annotation on the latest snapshot so
        # leadership sees historical context alongside the chart. Only
        # shown when a note exists (most snapshots are unannotated).
        latest_note_row = await db.execute_query(
            "SELECT notes FROM monthly_snapshots "
            "WHERE snapshot_date = $1 AND notes IS NOT NULL LIMIT 1",
            (last_date,),
            fetch_one=True,
        )
        if latest_note_row and latest_note_row["notes"]:
            embed.add_field(
                name=f"📝 Annotation ({utils.format_date(last_date)})",
                value=str(latest_note_row["notes"])[:1024],
                inline=False,
            )
